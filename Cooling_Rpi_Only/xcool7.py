...
pulse_counts = [0, 0]
flow_rates = [0.0, 0.0]        # [f1_esc_in, f2_motor_in]
pressure_bars = [0.0]          # only p1_esc_in
temperatures = [0.0, 0.0]      # [raw_t2, raw_t1]

# Temperature offsets
off_t2 = 9.2  # ESC inlet (ID-1 RS485)
off_t1 = 6.7  # Motor inlet (ID-3 RS485)
...
def flow_thread():
    while True:
        ...
        with data_lock:
            ...
            flow_rates[1] = calc_flow(pulse_counts[1], FLOW_FACTORS[1])  # f2_motor_in
            pulse_counts[0] = 0
            pulse_counts[1] = 0
        ...
        try:
            while True:
                with data_lock:
                    ...
                    f1_esc_in, f2_motor_in = flow_rates
                ...
                print(f"| ESC Inlet Pressure: {p1_esc_in:6.3f} Bar | "
                      ...
                      f"Motor Inlet Flow: {f2_motor_in:5.2f} Lpm | "
