...
# ------- LOADCELL PIN MAP (SAME AS ORIGINAL — DO NOT TOUCH) -------
...
# ==========================================================
# LOADCELL WORKER — AUTO ZERO/TARE ADDED HERE ONLY
# ==========================================================
def loadcell_worker(dout, sck, offset_unused, key):
    from hx711_150kg_module import HX711

    hx = HX711(dout, sck) 
    time.sleep(1)

    print(f"\n---------------- {key} ZEROING ----------------")
    hx.offset = hx.read_average(25)             # Auto zero offset
    print(f"{key} Offset Captured = {hx.offset}")
    print(f"------------------------------------------------\n")
    time.sleep(1.0)

    while running.is_set():
        try:
            w,_ = hx.get_weight()               # automatically weight-offset applied
...
# ==========================================================
# CAN/ESC TELEMETRY — UNCHANGED (PRINTS PRESERVED)
# ==========================================================
def esc_reader_transmitter():
    ...
    while running.is_set():
        ...
        if n > 0:
            for i in range(n):
                ...
                if m.ID == RESPONSE_ID and m.DataLen >= 3:
                    ...
                    if reg == REG_KT: shared['Kt'] = raw * 0.001
                    elif reg == REG_RPM: shared['RPM'] = round(raw * NOMINAL_RPM / 32767, 1)
                    elif reg == REG_RPM_CMD: shared['RPM_Cmd'] = round(raw * NOMINAL_RPM / 32767, 1)
                    elif reg == REG_TORQUE: shared['Torque'] = round(raw * 169.7 * shared['Kt'] / (32767 * sqrt(2)), 3)
                    elif reg == REG_MOTOR_TEMP: shared['Motor_Temp'] = round(0.0000003 * raw**2 + 0.0103 * raw - 127.43, 1)
                    elif reg == REG_CURRENT: shared['Current'] = round(raw * 0.138768, 2)
                    elif reg == REG_VDC:
        ...
        obj.ID = 0x106
        batt_v_raw = int(shared['Batt_V'] * 100)
        bus_v_raw  = int(shared['Bus_V'] * 100)
        power_raw  = int(shared['Power'] / 10)
        power_sign = 1 if power_raw >= 0 else -1
        
        obj.Data = (c_ubyte*8)(*pack('<HHhh', batt_v_raw, bus_v_raw, power_raw, power_sign))
        canlib.VCI_Transmit(VCI_USBCAN2, 0, 1, byref(obj), 1)
...
# ==========================================================
def main():
    print("Starting ideaForge RPi1 Thrust Rig Transmitter (AUTO ZERO ENABLED)...")
    procs = []
    ...
    for i, key in enumerate(LABELS):
        dout, sck = LOAD_CELL_PINS[i]
        p = multiprocessing.Process(target=loadcell_worker,
                                   args=(dout, sck, None, key))
        p.start()
        procs.append(p)

    print("\n*** Loadcells are zeroing — keep rig unloaded for 5 seconds ***\n")
