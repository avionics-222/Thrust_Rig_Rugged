...
# ideaForge RPi1 Thrust Rig — FINAL & COMPLETE
# All parameters correct + Individual T0/T90/T180/T270 + Cmd RPM in print
...
# Waveshare USB-CAN direct control
VCI_USBCAN2 = 4
STATUS_OK = 1
...
REG_VOUT        = 0x8A
REG_IGBT_TEMP   = 0x4A
REG_RPM_CMD     = 0x31  # Commanded RPM register
...
shared = multiprocessing.Manager().dict(
    ...
    Current=0.0, Vdc=0.0, Vout=0.0, Power=0.0, Kt=KT_DEFAULT
)
...
def esc_reader_transmitter():
    ...
    print("CAN initialized — Ch0: Read | Ch1: Transmit")
    ...
    print("\n" + "="*110)
    print(" RPi1 THRUST RIG — ALL PARAMETERS LIVE & TRANSMITTING ON CAN2")
    print("="*110 + "\n")
    ...
    while running.is_set():
        ...
        if time.time() - last_print >= 1.0:
            print(f"{datetime.now():%H:%M:%S} | "
                  ...
                  f"I:{shared['Current']:6.1f}A Vdc:{shared['Vdc']:5.1f}V Vout:{shared['Vout']:5.1f}V P:{shared['Power']:6.0f}W")
        ...
        data300 = []
        for k in LABELS:
            val = i16(shared[k], 1000)
            data300.extend([val & 0xFF, val >> 8])
        obj.Data = (c_ubyte*8)(*data300)
        canlib.VCI_Transmit(VCI_USBCAN2, 0, 1, byref(obj), 1)

        # 0x301
        obj.ID = 0x301
        obj.Data = (c_ubyte*8)(
            i16(total_kg,100) & 0xFF, i16(total_kg,100)>>8,
            i16(thrust_n,10) & 0xFF, i16(thrust_n,10)>>8,
            int(shared['RPM']) & 0xFF, int(shared['RPM'])>>8,
            i16(shared['Torque'],1000) & 0xFF, i16(shared['Torque'],1000)>>8
        )
        canlib.VCI_Transmit(VCI_USBCAN2, 0, 1, byref(obj), 1)
        ...
        obj.ID = 0x302
        obj.Data = (c_ubyte*8)(
            int(shared['Motor_Temp']*10), int(shared['ESC_Temp']*10),
            i16(shared['Current'],100) & 0xFF, i16(shared['Current'],100)>>8,
            i16(shared['Vdc'],100) & 0xFF, i16(shared['Vdc'],100)>>8, 0, 0
        )
        canlib.VCI_Transmit(VCI_USBCAN2, 0, 1, byref(obj), 1)
        ...
        obj.ID = 0x303
        obj.Data = (c_ubyte*8)(
            i16(shared['Vout'],100) & 0xFF, i16(shared['Vout'],100)>>8,
            int(shared['Power']) & 0xFF, int(shared['Power'])>>8, 0,0,0,0
        )
        canlib.VCI_Transmit(VCI_USBCAN2, 0, 1, byref(obj), 1)
